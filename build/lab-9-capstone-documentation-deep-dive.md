...
Prerequisites: [`00-shared-setup.md`](00-shared-setup.docx) and Labs 1–8.
...
> **Collapsed section in the original — Spoiler — the resource graph of the reference solution. Do not open until you have attempted the challenge.**
>
> In Word this cannot be hidden, so it is shown in full below.
> Skip past it if you are attempting the exercise unaided.
...
> **Collapsed section in the original — Hint 1 — the shape of the problem**
>
> In Word this cannot be hidden, so it is shown in full below.
> Skip past it if you are attempting the exercise unaided.
...
> **Collapsed section in the original — Hint 2 — the bucket is not enough**
>
> In Word this cannot be hidden, so it is shown in full below.
> Skip past it if you are attempting the exercise unaided.
...
> **Collapsed section in the original — Hint 3 — you have a bucket and a website config, and you get 403**
>
> In Word this cannot be hidden, so it is shown in full below.
> Skip past it if you are attempting the exercise unaided.
...
> **Collapsed section in the original — Hint 4 — the tutorial you found says acl = "public-read"**
>
> In Word this cannot be hidden, so it is shown in full below.
> Skip past it if you are attempting the exercise unaided.
...
- `terraform destroy` removes everything, and the tag-based backstop query from
  [`00-shared-setup.md` §6](00-shared-setup.docx) returns nothing.
...
3. **Version the site.** Enable bucket versioning, change `index.html`, apply, then retrieve the
   ...
   [Lab 8](lab-8-loops-backends.docx) gives you content rollback here.
...
4. **Serve a real directory.** Replace the two inline `aws_s3_object` resources with a `for_each`
   ...
   per file extension with a lookup map. This combines [Lab 7](lab-7-functions-data-types.docx)'s
...
5. **Write the module.** Package this as a reusable module with the interface discipline from
   [Lab 5](lab-5-modules-workspaces.docx) — typed variables in, `website_endpoint` out — and call it
